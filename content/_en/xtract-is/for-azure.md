...
Instead of deploying those packages to an on-prem SSIS server, those packages are deployed to an Azure-SSIS IR to be scheduled and run in the Azure cloud.

### Architecture

![XISforAzure_Architecture](/img/content/xis/Xtract_IS_for_Azure.png){:class="img-responsive"}

Xtract IS for Azure makes sure that Azure-SSIS IR supports SSIS packages containing Xtract IS components for connecting to and extracting data from SAP. The SAP system needs to be accessible from the Azure-SSIS IR, for instance through a VPN tunnel or a self-hosted Integration Runtime.
